...
with gr.Blocks() as app:
    gr.Markdown(
        """
        # Welcome!
        Select a Hugging Face model or tag and deploy it on a port
    ...
    btn_dl.click(lambda: gr.update(label="Building vLLM container",visible=True), None, create_response).then(docker_api_create,inputs=[model_dropdown,selected_model_pipeline_tag,port_model,port_vllm],outputs=create_response).then(refresh_container_list, outputs=[container_state]).then(lambda: gr.Timer(active=True), None, timer_dl).then(lambda: gr.update(visible=True), None, create_response).then(lambda: gr.update(visible=True), None, btn_interface)
